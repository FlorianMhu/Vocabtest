...
//Gestion du param�tre de luminosit�
...
function splittxt(file,arr) {
    ...
    for (var i = 0; i < arr[0].length; i++) {
        //Assignation de num � chaque couple de mot
        arr[2][i] = i;            
    }     
...
function verification_txt(tabletxt) {
    ...
    for (var lang = 0; lang < 2; lang++) {
        for (i = 0; i < tabletxt[0].length; i++) {
            //Supression des sauts de ligne �ventuels et espaces en d�but de cha�ne
            ...
            //Suppression des espaces en fin de cha�ne de caract�re
...
function load_data() {
    if (sessionStorage.butnumber) {
        var arr = [];
        for (var i = 0; i < 4; i++) {//4 colonne de tableau: mots anglais, mots fran�ais, le num de ligne pour le mot, et la langue (0-> anglais , 1 ->fran�ais)
            arr[i] = [];
        }
        ...
        sessionStorage.setItem("tableau", JSON.stringify(arr));//Upload des donn�es dans le web storage

        window.open("../index.html", "_self");//Ouverture page pr�c�dente
